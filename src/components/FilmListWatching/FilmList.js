...
function FilmListWatching() {
  ...
  useEffect(() => {
    axios
      .get(
        "https://api.themoviedb.org/3/movie/popular?api_key=070460ee0b557df99bd8fd941d183e23"
      )
  ...
  const handleMuteItem = () => {
    document
      .querySelector(".home__item-img .mute__icon")
      .classList.toggle("mute");
  };
  const settings = {
    // className: "center",
    infinite: true,
        ...
        {movieList &&
          movieList.map((movie) => (
            <a className="home__slider-item" href="/phim/detail">
              <img
                className="home__slider-item-img"
                src={movie ? baseUrl.concat(movie.poster_path) : ""}
                alt=""
              />
